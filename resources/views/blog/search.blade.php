    ...
    <div class="background-image grid grid-cols-1 m-auto">
        <div class="flex text-gray-100 pt-10">
            <div class="m-auto pt-4 pb-16 sm:m-auto w-4/5 block text-center">
                ...
                <form action="{{ route('searchBlog') }}" method="GET" class="mt-7">
                ...
                <form>
                    @if (Auth::check())
                        <div class="pt-15 w-4/5 m-auto">
                            <a href="/blog/create?topicId={{ $topic_id }}"
    ...
    <div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200">
                ...
                <div>
                    ...
                    <div class="flex justify-between">
                        ...
                        @if (isset(Auth::user()->id) && Auth::user()->id == $post->user_id)
                            <div class="flex">
                                <a href="{{ route('blog.edit', [$post->slug, 'topicId' => $topic_id]) }}"
                                    class="colored_button uppercase bg-transparent text-gray-100 text-xs font-extrabold py-3 px-5 rounded-3xl">
                                    Edit
                                </a>
                                ...
                                <form action="{{ route('blog.destroy', $post->slug) }}" method="POST">
                                    @csrf
                                    @method('delete')
                                    <input type="hidden" name="topics_id" value="{{ $topic_id }}">
                                    <button
                                        class="bg-red-500 uppercase bg-transparent text-gray-100 text-xs font-extrabold py-3 px-5 rounded-3xl ml-2"
                                        type="submit">
                                        Delete
                                    </button>
                                </form>
        ...
        @else
            <div class="pt-15 w-4/5 m-auto text-center">
                <h2 class="bg-red-500 uppercase bg-transparent text-gray-100 text-xs font-extrabold py-3 px-5 rounded-3xl">
                    No Posts Found!
                </h2>
            </div>
        @endif
